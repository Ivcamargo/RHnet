...
- **Recruitment & Selection Module**: Manages hiring workflow from job openings to digital onboarding. Includes job postings, candidate database, application tracking (status flow: applied → hired), and secure digital onboarding links. All data is company-scoped. Features sophisticated candidate filtering with weighted scoring system:
  ...
  - **Public Jobs Page** (`/vagas`): Career portal displaying all published job openings with company branding, location, salary range, employment type, and experience level. Features client-side filtering by location and employment type. Public endpoints (`/api/public/jobs`, `/api/public/jobs/:id`, `/api/public/apply`) filter by status='published' to show only active listings. Job status values: draft, published, closed, filled. Frontend displays companyName (joined from companies table) and salaryRange (varchar field) for complete job information.
